...
const PaymentSchema = Yup.object().shape({
  ...
  cardNumber: Yup.string().when('type', {
    is: (v: any) => v !== 'pix',
    then: (schema) => schema.required('Obrigatório').test('luhn', 'Número de cartão inválido', v => v ? validCardNumber(v) : false),
    otherwise: (schema) => schema
  }),
  expiry: Yup.string().when('type', {
    is: (v: any) => v !== 'pix',
    then: (schema) => schema.required('Obrigatório').matches(/^(0[1-9]|1[0-2])\/(?:[0-9]{2})$/, 'Formato MM/YY').test('exp', 'Cartão vencido', (val: any) => {
      if (!val) return false;
      const [mm, yy] = val.split('/');
      const expiry = new Date(Number('20' + yy), Number(mm) - 1 + 1, 1);
